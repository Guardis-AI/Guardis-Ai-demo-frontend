import { Component, ElementRef, Input, OnInit, ChangeDetectionStrategy , AfterViewInit} from '@angular/core';
...
export class LivestreamComponent implements OnInit,AfterViewInit {
  ...
  searchText:any;playbacklist :any;stream='';vdata :any;  cameraType:'01';
 ...
 devicelist = [];
  ...
  ngOnInit() {
    const data = {"p_user_id":localStorage.getItem('userId'),"p_device_token":"","p_device_type":"","p_port":"","login_user_id":localStorage.getItem('userId')}
  ...
  ngAfterViewInit() {
    this.player = videojs(document.getElementById('sxmvideo'));
      this.devicelist.map(function(value,index) {
        ...
        document.getElementById('playbkvideo_'+value.device_id).style.height = '130px';
       });
